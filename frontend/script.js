...
// Display real-time privacy issues in the input field (with red underline overlay)
    ...
    // Extract AI privacy suggestions for each detected PII
    getPrivacySuggestions() {
        ...
        this.state.analyzedLog.forEach((entry, index) => {
            const messageSuggestions = {
                message_index: index,
                user_suggestions: null,
                bot_suggestions: null
            };
            
            // Extract user message suggestions
            if (entry.userPrivacy && entry.userPrivacy.privacy_issue) {
                messageSuggestions.user_suggestions = {
                    type: entry.userPrivacy.type,
                    original_text: entry.user,
                    suggestion: entry.userPrivacy.suggestion || null,
                    placeholder: entry.userPrivacy.safer_versions?.replacing || null,
                    blurred_data: entry.userPrivacy.safer_versions?.abstraction || null
                };
            }
            
            // Extract bot message suggestions
            if (entry.botPrivacy && entry.botPrivacy.privacy_issue) {
                messageSuggestions.bot_suggestions = {
                    type: entry.botPrivacy.type,
                    original_text: entry.bot,
                    suggestion: entry.botPrivacy.suggestion || null,
                    placeholder: entry.botPrivacy.safer_versions?.replacing || null,
                    blurred_data: entry.botPrivacy.safer_versions?.abstraction || null
                };
            }
            
            // Only add entries that have actual suggestions
            if (messageSuggestions.user_suggestions || messageSuggestions.bot_suggestions) {
